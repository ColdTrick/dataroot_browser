<?php 
	...
	$root_dir = str_replace("//", "/", $CONFIG->dataroot . $current_dir);
	...
	if($dir_data !== false){
		...
		if($current_dir != "/"){
			...
			echo " <a class='dataroot_browser_up' href='" . $vars["url"] . "pg/dataroot_browser?dir=" . urlencode($parent_dir) . "'></a>";
		}
		echo "<br /><br />";
		foreach($dir_data as $file){
			if($file != "." && $file != ".."){
				if(is_dir($root_dir . $file)){
					...
					$dir_items .= "<a href='" . $vars["url"] . "pg/dataroot_browser?dir=" . urlencode($current_dir . $file) . "'>" . $file . "</a>";
					$dir_items .= "</td>";
					$dir_items .= "</tr>";
				} else {
					...
					$file_items .= "<td>";
					$file_items .= "<a href='" . elgg_add_action_tokens_to_url($vars["url"] . "action/dataroot_browser/delete_file?file=" . urlencode($current_dir . $file)) . "' onclick='return confirm(\"" . addslashes(elgg_echo("question:areyousure")) . "\");' ><span class='dataroot_browser_delete' ></span></a>";
					$file_items .= "</td>";
					$file_items .= "</tr>";
				}
			}
		...
		echo "<table id='dataroot_browser_list'>";
		echo "<tr>";
